...
function IsAuthenticated({ auth }: { auth?: string }) {
  if (!auth) {
    return (
      <>
        <Link
...
export default function Navbar({ auth }: { auth?: string }) {
  return (
            ...
            <div className="flex flex-col gap-0.5 py-2 md:flex-row md:items-center md:gap-1 md:py-0">
              <div className="grow">
                <div className="ml-5 flex flex-col gap-0.5 md:flex-row md:items-center md:justify-start md:gap-1">
                  <Link
                    className="flex items-center rounded-lg p-2 text-sm text-gray-950 hover:font-semibold"
                    to={"/"}
                  >
                    <svg
